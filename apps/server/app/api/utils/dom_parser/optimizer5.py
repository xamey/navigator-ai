...
class DOMXPathMapper:
    """Creates compact DOM representation using XPath mapping to minimize tokens."""
    ...
    def create_minimal_dom(self, dom_hashmap: Dict) -> Tuple[str, Dict]:
        ...
        # Create reverse mapping (id -> xpath) for easier lookup
        id_to_xpath_map = {f"e{id_num}": xpath for xpath,
                           id_num in self.xpath_to_id_map.items()}

        return minimal_html, id_to_xpath_map
    ...
    def _get_xpath_id(self, xpath: str) -> str:
        """Get or create a short ID for an XPath."""
        if not xpath:
            return ""

        if xpath not in self.xpath_to_id_map:
            self.xpath_to_id_map[xpath] = self.next_id
            self.next_id += 1

        return f"e{self.xpath_to_id_map[xpath]}"
    ...
    def _generate_minimal_html(self, grouped_elements: Dict, dom_hashmap: Dict) -> str:
        ...
        for tag_name, elem_ids in sorted(grouped_elements.items()):
            ...
            for elem_id in elem_ids:
                ...
                # Add mapped XPath ID instead of full XPath
                xpath = self._get_attr(element, 'xpath', '')
                xpath_id = self._get_xpath_id(xpath)
                if xpath_id:
                    element_output += f" data-eid=\"{xpath_id}\""
...
def generate_xpath_map_and_minimal_dom_for_llm(dom_state):
    """Generate minimal DOM representation with XPath mapping for LLM consumption."""
    mapper = DOMXPathMapper(
        max_text_length=200,  # Allow enough text for identification
