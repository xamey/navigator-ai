...
class CompleteCompactMapper:
    """
    Creates a highly token-efficient representation of DOM for LLM consumption.
    Provides both XPath and CSS selector maps for flexible element targeting.
    """
    ...
    def _get_other_interactive_elements(self, dom_hashmap: Dict, exclude_ids: Set[str]) -> List[str]:
        ...
        return [elem_id for elem_id in self.interactive_elements if elem_id not in exclude_ids]
        ...
        if parent_id:
            ...
            if parent:
                ...
                for child_id in parent_children:
                    child = dom_hashmap.get(str(child_id) if isinstance(child_id, int) else child_id)
                ...
                if siblings:
                    ...
                    position = siblings.index(elem_id) + 1
                    if position > 0:
...
def generate_complete_compact_dom(dom_state):
    """Generate a complete compact DOM representation with both XPath and selector maps."""
    mapper = CompleteCompactMapper()
