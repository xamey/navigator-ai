// Logger helper
const logger = {
    debug: (message: string, ...data: any[]) => console.debug(`[Filters] ${message}`, ...data),
    error: (message: string, error?: any) => console.error(`[Filters] ${message}`, error)
};
...
export const isTextNodeVisible = (textNode: Text): boolean => {
    try {
        ...
        logger.debug(`Text node visibility: ${isVisible} (hasSize: ${hasSize}, inViewport: ${isInViewport}, parentVisible: ${parentVisible})`);
        return isVisible;
    } catch (error) {
...
export const isInteractiveElement = (element: Element) => {
    try {
        ...
        logger.debug(`Element interactivity: ${isInteractive} (ariaProps: ${hasAriaProps}, clickHandler: ${hasClickHandler}, clickListeners: ${hasClickListeners}, draggable: ${isDraggable})`);
