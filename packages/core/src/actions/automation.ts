...
export class AutomationHandler implements IAutomationHandler {
  ...
  private initializeCursor(): void {
    ...
    try {
      ...
      if (this.debugMode) {
        console.log(
          "Navigator AI cursor initialized",
          this.cursorManager.cursorElement
        );
  ...
  async executeAction(
    ...
    try {
      ...
      if (!element && action.type !== "navigate" && action.type !== "url") {
        ...
        if (retryCount < 2) {
          console.log(
            `Action failed, waiting and retrying (attempt ${retryCount + 1}/2)`
          );
        ...
        return {
          success: false,
          message: `Element not found for action type: ${action.type}${
            action.selector ? `, selector: ${action.selector}` : ""
          }${action.xpath_ref ? `, xpath: ${action.xpath_ref}` : ""}${
      ...
      switch (action.type) {
        ...
        case "url":
          if (!action.url)
            return {
              success: false,
              message: `Action type '${action.type}' could not be executed as url was not provided`,
            };
      ...
      return {
        success: true,
        message: `Action type '${action.type}' executed successfully${
          action.selector ? ` on selector: ${action.selector}` : ""
        }${action.xpath_ref ? ` on xpath: ${action.xpath_ref}` : ""}${
          action.element_id ? ` on element_id: ${action.element_id}` : ""
        }${action.url ? ` with url: ${action.url}` : ""}${
          action.text ? ` with text input` : ""
        }`,
      };
    } catch (error) {
      ...
      if (retryCount < 2) {
        console.log(
          `Action failed with error, retrying (attempt ${retryCount + 1}/2)`
        );
        await new Promise((resolve) => setTimeout(resolve, 2000));
        return this.executeAction(action, retryCount + 1);
      }
      ...
      return {
        success: false,
        message: `Failed to execute action type: '${action.type}'${
          action.selector ? `, selector: ${action.selector}` : ""
        }${action.xpath_ref ? `, xpath: ${action.xpath_ref}` : ""}${
          action.element_id ? `, element_id: ${action.element_id}` : ""
        }${action.url ? `, url: ${action.url}` : ""}${
          action.text ? `, text length: ${action.text.length}` : ""
        }`,
      };
    }
  }
  ...
  async executeActions(actions: Action[]): Promise<ExecuteActionResult[]> {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      if (!currentAction) {
        results.push({
          success: false,
          message: `Action at index ${i} is undefined`,
        });
        continue;
      }

      console.log(
        `Executing action ${i + 1} of ${actions.length}:`,
        currentAction.type
      );

      try {
        const success = await this.executeAction(currentAction);
        ...
        if (!success) {
          console.error(
            `Action ${i + 1} (${
              currentAction.type
            }) failed. Continuing with next action.`
          );
        } else {
          console.log(
            `Action ${i + 1} (${currentAction.type}) completed successfully`
          );
        }
        ...
        console.error(
          `Unexpected error in action ${i + 1} (${currentAction.type}):`,
          error
        );
        results.push({
          success: false,
          message: `Unexpected error in action ${i + 1} (${
            currentAction.type
          })${
            ...
            currentAction.xpath_ref ? `, xpath: ${currentAction.xpath_ref}` : ""
          }${
            currentAction.element_id
              ? `, element_id: ${currentAction.element_id}`
              : ""
          }: error: ${error}`,
        });
        await new Promise((resolve) => setTimeout(resolve, 1000));
